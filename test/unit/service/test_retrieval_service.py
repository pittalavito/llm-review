...
class FakeStoreService:
    ...
    @staticmethod
    def compute_doc_id(relative_path, strategy, strategy_version):
        return RedisRagIndexRepository.compute_doc_id(relative_path, strategy, strategy_version)
    ...
    def list_indexed_papers(self) -> list[str]:
        return sorted({index.paper_path for index in self.store.values()})
...
class TestRetrieveContext:
    def test_full_context_returns_the_paper(self, setup):
        service, _, _ = setup
        context = service.retrieve_context("p.txt", "ignored", RagStrategy.FULL_CONTEXT, "v1")
    ...
    def test_embedding_path_works_with_default_mock_embedder(self, setup):
        service, _, _ = setup
        context = service.retrieve_context("p.txt", "imagenet", RagStrategy.EMBEDDING, "v1")
        assert isinstance(context, str) and context

    def test_missing_paper_raises_not_found(self, setup):
        service, _, _ = setup
        with pytest.raises(NotFoundError):
            service.retrieve_context("nope.txt", "q", RagStrategy.FULL_CONTEXT, "v1")


class TestIndexing:
    def test_index_paper_persists_and_returns_info(self, setup):
        service, store, _ = setup
        info = service.index_paper("p.txt", RagStrategy.BM25, "v1")
        assert isinstance(info, IndexInfo)
        assert store.save_calls == 1
        assert any("+bm25-v1" in doc_id for doc_id in store.store)  # readable composite key

    def test_strategies_coexist_for_one_paper(self, setup):
        service, store, _ = setup
        service.index_paper("p.txt", RagStrategy.FULL_CONTEXT, "v1")
        service.index_paper("p.txt", RagStrategy.BM25, "v1")
        assert len(store.store) == 2  # no overwrite across strategies
        assert service.list_indexed() == ["p.txt"]  # listed once
    ...
    def test_uses_cache_then_rebuilds_when_file_changes(self, setup):
        ...
        service.retrieve_context("p.txt", "q", RagStrategy.FULL_CONTEXT, "v1")
        assert store.save_calls == 1  # cache hit — not rebuilt

        (papers / "p.txt").write_text("a much longer brand new body about transformers and attention", encoding="utf-8")
        context = service.retrieve_context("p.txt", "q", RagStrategy.FULL_CONTEXT, "v1")
