...
class PaperService:
    ...
    def save_paper(self, paper: Paper, data: bytes, file_format: str, authors: list[Author] | None = None) -> Paper | None:
        """Create the catalog row (uid paper_id), store the file and link the
        authors. None when a paper with the same id already exists."""
        return self._store_service.save_paper(paper, data, file_format, authors=authors)
    ...
    def create_from_openreview(self, request: CreateOpenReviewPaperRequest) -> Paper | None:
        ...
        if not request.file_bytes.startswith(b"%PDF"):
            raise ValidationError(
                ...
                "challenge page saved as .pdf. Re-download the paper and retry."
            )
