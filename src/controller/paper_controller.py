...
def create_paper(
    ...
    indexing in background — the response does not wait for it. The paper_id
    is generated by the BE; the file format comes from ``file_name``."""
    file_format = request.file_name.rsplit(".", 1)[-1].lower() if "." in request.file_name else ""
    if file_format not in ("pdf", "txt"):
        raise HTTPException(status_code=400, detail="Unsupported file type. Use .pdf or .txt files.")
    saved = paper_service.save_paper(request.paper, request.file_bytes, file_format, authors=request.authors)
    if saved is None:
        raise HTTPException(status_code=409, detail="A paper with this id already exists.")
    background_tasks.add_task(retrieval_service.multi_strategy_indexed, saved.paper_id)
    return CreatePaperResponse.from_response(saved)
